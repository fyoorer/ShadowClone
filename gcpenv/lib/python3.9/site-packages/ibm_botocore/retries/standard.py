...
The key base classes uses for retries:
    ...
    * ``RetryHandler`` - The bridge between ibm_botocore's event system
...
based API used by ibm_botocore.
...
from ibm_botocore.retries import quota, special
from ibm_botocore.retries.base import BaseRetryableChecker, BaseRetryBackoff
...
class RetryHandler:
    """Bridge between ibm_botocore's event system and this module.

    This class is intended to be hooked to ibm_botocore's event system
    as an event handler.
    """
...
class RetryEventAdapter:
    ...
    is event based and used in ``ibm_botocore.endpoint``.  The interface has
...
class RetryContext:
    ...
    def __init__(
        ...
        # This is an instance of ibm_botocore.awsrequest.AWSResponse.
