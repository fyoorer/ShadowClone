...
from ibm_botocore import UNSIGNED, xform_name
from ibm_botocore.auth import AUTH_TYPE_MAPS, HAS_CRT
from ibm_botocore.crt import CRT_SUPPORTED_AUTH_TYPES
from ibm_botocore.endpoint_provider import EndpointProvider
...
from ibm_botocore.utils import ensure_boolean, instance_cache
...
class EndpointResolver(BaseEndpointResolver):
    ...
    def _create_tag_list(self, use_dualstack_endpoint,
                         # IBM Unsupported
                         # use_fips_endpoint
                         ):
        tags = []
...
class EndpointRulesetResolver:
    ...
    def construct_endpoint(
        ...
        try:
            ...
            ibm_botocore_exception = self.ruleset_error_to_ibm_botocore_exception(
                ex, provider_params
            )
            if ibm_botocore_exception is None:
                raise
            else:
                raise ibm_botocore_exception from ex
        ...
        # Multi-valued headers are not supported in ibm_botocore. Replace the list
    ...
    def auth_schemes_to_signing_ctx(self, auth_schemes):
        ...
        if self._requested_auth_scheme is not None:
            try:
                ...
                # after accounting for naming differences between ibm_botocore and
                ...
                # exception, instead default to the logic in ibm_botocore
                # customizations.
                return None, {}
        else:
            try:
                ...
                if fixable_with_crt:
                    raise MissingDependencyException(
                        ...
                        ' Use pip install ibm_botocore[crt] before proceeding.'
                    )
                else:
    ...
    def _does_botocore_authname_match_ruleset_authname(self, ibm_botoname, rsname):
        """
        ...
        * The s3 and s3control rulesets don't distinguish between v4[a] and
          s3v4[a] signers
        * The v2, v3, and HMAC v1 based signers (s3, s3-*) are ibm_botocore legacy
        ...
        * s3-presign-post, sigv4
        ...
        ibm_botoname = ibm_botoname.split('-')[0]
        if ibm_botoname != 's3' and ibm_botoname.startswith('s3'):
            ibm_botoname = ibm_botoname[2:]
        return rsname == ibm_botoname

    def ruleset_error_to_botocore_exception(self, ruleset_exception, params):
        """Attempts to translate ruleset errors to pre-existing ibm_botocore
