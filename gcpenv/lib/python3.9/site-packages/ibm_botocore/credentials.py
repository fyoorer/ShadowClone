...
from ibm_botocore.compat import compat_shell_split, total_seconds
...
from ibm_botocore.tokens import SSOTokenProvider
...
class EnvProvider(CredentialProvider):
    ...
    def load(self):
        ...
        ibm_api_key_id = self.environ.get(self._mapping['ibm_api_key_id'], '')
        ...
        for filename in potential_locations:
            ...
            if 'Credentials' in config:
                ...
                if self.IBM_COS_API_KEY_ID in credentials:
                    ...
                    logger.info(
                        "Found credentials in ibm_boto config file: %s", filename
                    )
                    access_key, secret_key = self._extract_creds_from_mapping(
